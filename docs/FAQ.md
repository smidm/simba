...
2. There is several `Video1` rows in your `project_folder/logs/video_info.csv` file. SimBA happens to find them all, can't decide which one is the correct one, and it breakes. Make sure you only have one row per video in your project in your `project_folder/logs/video_info.csv` file. 
